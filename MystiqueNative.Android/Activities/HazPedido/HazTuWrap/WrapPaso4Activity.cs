...
using Humanizer;
using MystiqueNative.Droid.HazPedido;
...
namespace MystiqueNative.Droid.HazPedido.HazTuWrap
{
    ...
    public class WrapPaso4Activity : BaseActivity
    {
        ...
        private void ButtonTerminar_Click(object sender, System.EventArgs e)
        {
            if (ViewModel.Instance.HaCompletadoCuartoPaso)
            {
                if (_pedido == null)
                {
                    ...
                    var dialog = new TortillaWrapDialogFragment();
                    dialog.DialogClosed += Dialog_DialogClosed;
                    dialog.Show(SupportFragmentManager, nameof(TortillaWrapDialogFragment));
                }
                else
                {
        ...
        private void Dialog_DialogClosed(object sender, TortillaDialogEventArgs e)
        {
            if (e == null || e.TipoTortilla == TortillaWrap.NoDefinida) return;
            ViewModel.Instance.SeleccionarTortilla(e.TipoTortilla);
            ViewModel.Instance.SeleccionarAderezo(AderezoEnsalada.EnEnsalada);
            //StartActivityForResult(typeof(WrapPaso5Activity), 1);
            TerminarEnsalada();
        }
