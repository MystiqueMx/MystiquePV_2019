using FacturacionTDCAPI.Helpers.PDF.ViewModels;
using QRCoder;
using System;
...
namespace FacturacionTDCAPI.Helpers.PDF.Helpers
{
    public class GeneracionPdfHelper
    {
        public PdfFactura ObtenerViewModelFactura(string documentoTimbrado, string cadenaOriginal)
        {
            ...
            datosFactura.UUID = "5291DC3C-B449-4CC5-B665-DD81384BC126";
            return datosFactura;
        }
        ...
        private static PdfFactura LlenarFactura(string xml)
        {
            ...
            return new PdfFactura
            {
                ...
                noCertificado = GetFirstValue(doc, "cfdi:NoCertificado", "noCertificado"),
                ...
                IVATrasladado = GetTotalImpuestosTrasladados(doc, "cfdi:Impuestos", "TotalImpuestosTrasladados"),
        ...
        private static string GetTotalImpuestosTrasladados(XmlDocument doc, string tag, string atributo)
        {
            #region GetFirstValue
            var valor = string.Empty;
            try
            {
                var xmlTagCollection = doc.GetElementsByTagName(tag);
                if (xmlTagCollection != null)
                {
                    foreach (XmlNode tagxml in xmlTagCollection)
                    {
                        var atributoxml = tagxml.Attributes[atributo];
                        if (atributoxml != null)
                        {
                            return atributoxml.Value;
                        }
                    }
                }
            }
