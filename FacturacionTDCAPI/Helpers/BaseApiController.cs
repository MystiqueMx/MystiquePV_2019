...
public abstract class BaseApiController : ApiController
{
    ...
    #region Contexto
    private FacturacionTDCDevEntities _context;
    #if DEBUG
    protected FacturacionTDCDevEntities Contexto
    {
        get
        {
            if (_context != null) return _context;
            _context = new FacturacionTDCDevEntities();
            _context.Database.Log = sql => Logger.Debug(sql);
            return _context;
        }
    }
    #else
        public FacturacionTDCDevEntities Contexto => _context ?? (_context = new FacturacionTDCDevEntities());
    #endif
    #endregion
    ...
    protected static ErrorCodeResponseBase RespuestaErrorFacturacion(long id) => new ErrorCodeResponseBase
    {
        Message = "Ocurrió un error al facturar el consumo solicitado",
